...
// Falling with earthquake

#define VORONOIPOINTS 5
#define FLOORS 5
#define FLOORX 9.0
#define FLOORY 9.0
...
#define COLUMNSX 5.0
#define COLUMNSY 5.0
#define COLUMNSIZE 2.0
#define COLUMN_CONSISTENCY 0.05

#define START_EARTHQUAKE 10
#define EARTHQUAKE_TIME 20
#define EARTHQUAKE_X 700
#define EARTHQUAKE_Y 700
#define EARTHQUAKE_Z 50
...
//#define COLUMNSY 5.0
//#define COLUMNSIZE 2.0
//#define COLUMN_CONSISTENCY 0.01
//
//#define START_EARTHQUAKE 10
//#define EARTHQUAKE_TIME 20
//#define EARTHQUAKE_X 700
...
//#define COLUMN_CONSISTENCY 0.05
//
//#define START_EARTHQUAKE 2
//#define EARTHQUAKE_TIME 20
//#define EARTHQUAKE_X 70
//#define EARTHQUAKE_Y 70
//#define EARTHQUAKE_Z 50
...
{
    friend void placeColumn(VoronoiFractureDemo* ref, btVector3 size, btVector3 location);
    ...
    virtual void stepSimulation(float deltaTime)
    {
        if (m_dynamicsWorld)
        {
            ...
            if (currentTime > START_EARTHQUAKE && currentTime < START_EARTHQUAKE + EARTHQUAKE_TIME)
            {
                printf("EARTHQUAKE Started\n");
                btTransform tr;
                groundRB->getMotionState()->getWorldTransform(tr);
                groundRB->applyForce(btVector3(sin(currentTime) * EARTHQUAKE_X,
                                               sin(currentTime) * EARTHQUAKE_Z,
                                               sin(currentTime) * EARTHQUAKE_Y), tr.getOrigin());
...
void placeColumn(VoronoiFractureDemo* ref, btVector3 size, btVector3 location)
{
    ...
    ref->m_perfmTimer.reset();
    ref->voronoiBBShatter(points, bbmin, bbmax, bbq, bbt, COLUMN_CONSISTENCY);
    printf("Total Time: %f seconds\n", ref->m_perfmTimer.getTimeMilliseconds()/1000.);
    
    for (int i=ref->m_dynamicsWorld->getNumCollisionObjects()-1; i>=0 ;i--)
    {
        btCollisionObject* obj = ref->m_dynamicsWorld->getCollisionObjectArray()[i];
        obj->getCollisionShape()->setMargin(CONVEX_MARGIN+0.01);
    }
    ref->m_dynamicsWorld->performDiscreteCollisionDetection();
...
//    ref->attachFixedConstraints();
    
//    ref->m_guiHelper->autogenerateGraphicsObjects(ref->m_dynamicsWorld);
}

btRigidBody* placeFloor(VoronoiFractureDemo* ref, float mass, btVector3 size, btVector3 location)
{
    ...
    ref->m_dynamicsWorld->addRigidBody(body);
    return body;
}
...
//	btCollisionShape* groundShape = new btStaticPlaneShape(btVector3(0,1,0),50);
    ...
    groundRB = placeFloor(this, 1, btVector3(500,1,500), btVector3(0,0,0));
    ...
    for (int32_t i = 0; i < FLOORS; ++i)
    {
        ...
        for (int32_t x = -((COLUMNSX - 1) / 2); x <= ((COLUMNSX - 1) / 2); ++x)
        {
            float xx = x / ((COLUMNSX - 1) / 2);
            
            for (int32_t y = -((COLUMNSY - 1) / 2); y <= ((COLUMNSY - 1) / 2); ++y)
            {
                float yy = y / ((COLUMNSY - 1) / 2);
                
                placeColumn(this, btVector3(COLUMNSIZE,5,COLUMNSIZE), btVector3(xx * (FLOORX - 2), 3.55 + i * 6,yy * (FLOORY - 2)));
            }
        }
        
        placeFloor(this, 10, btVector3(FLOORX,0.25,FLOORY), btVector3(0,6.05 + i * 6.01f,0));
